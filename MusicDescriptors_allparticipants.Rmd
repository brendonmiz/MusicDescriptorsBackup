...
#              !which(partcols$nationality %in% findtheamericans))
              

partcols$nationality[which(partcols$nationality %in% findtheamericans)] <- "US"

#adjcols <- adjcols[rows2use,]
#partcols <- partcols[rows2use,]

partcols$nationality[partcols$nationality == "FR" | partcols$nationality == "FR-BE"] <- "France"

partcols$nationality[partcols$nationality != "US" & partcols$nationality != "France"] <- "Other"

n.rowvec <- list(US = which(partcols$nationality == "US"),
                 FR = which(partcols$nationality == "France"),
                 Other = which(partcols$nationality == "Other"))
...
```{r savethedata}
adj.cont.table.all <- apply(thebigone,c(1,2),sum)

adj.catadata.list.all <- list("adjbrick" = thebigone,
                          "adjcontingency" = adj.cont.table.all,
                          "partcols" = partcols,
                          "adjcols" = adjcols,
                          "listofwords" = listofwords
                          )

save(adj.catadata.list.all, file = "adj.catadata.list.all.RData")
```
...
natcols <- c(wes_palettes$Darjeeling1[1],
             wes_palettes$Darjeeling2[2],
             wes_palettes$Darjeeling1[3])
nat.factor <- partcols$nationality
col4nat <- nat.factor
col4nat[col4nat == "US"] <- natcols[1]
col4nat[col4nat == "France"] <- natcols[2]
col4nat[col4nat == "Other"] <- natcols[3]
...
adjbrick <- adj.catadata.list.all$adjbrick
...
a.000.zeMapCATA <- makeggHeatMap4CT(
                    CTstared(adj.cont.table.all,zeStars,'after'),
                    colorAttributes = col4adjs,
                    colorProducts = ex.cols, 
...
```{r}
adj.contingency <- adj.catadata.list.all$adjcontingency
adjbrick <- adj.catadata.list.all$adjbrick
...
ngroups <- length(unique(ex.cols))

mus.tree4ex.bydimdesign <- factoextra::fviz_dend(fit,  k = ngroups, 
                        k_colors = unique(ex.cols),
                        label_cols = col4excerpts[-17],
...
adj.design.all <- list("thedesign" = adjdesign,
                  "col4exgrp" = unique(col4exgrp),
                  "ex.cols" = col4exgrp
                  )

save(adj.design.all, file = "adjdesignall.RData")
```

```{r kmeans4words}

D.w <- dist(adjsym.cares$ExPosition.Data$fj, method = "euclidean")
...
cols <- list("g.gc" = gencols,
             ...
             "t.gc" = traincols,
             "t.oc" = col4train,
             "adj.gc" = unique(col4words),
             "adj.oc" = col4words,
             "ex.gc" = unique(col4exgrp),
             "ex.oc" = col4exgrp,
             "n.gc" = unique(col4nat),
             "n.oc" = col4nat
             )

save(cols, file = "adjectivecolorsall.RData")
...
n.elliadj <- MakeCIEllipses(BootCadj.N$BootCube[,1:2,],
                            ...
                            col = cols$n.gc[order(unique(partcols$nationality))])
```
...
n.rv.means <- createFactorMap(n.adjmeans,
                              ...
                              col.points =  cols$n.gc[order(unique(partcols$nationality))],
                              alpha.points = 1, # no transparency
                              display.points = TRUE,
                              pch = 19, cex = 5,
                              display.labels = TRUE,
                              col.labels = cols$n.gc[order(unique(partcols$nationality))], 
                              text.cex = 4,font.face = "bold",
                              font.family = "sans", col.axes = "darkorchid", 
                              alpha.axes = 0.2, width.axes = 1.1, 
                              col.background = adjustcolor("lavender", alpha.f = 0.2), 
...
mds3g = "MDS: Distance analysis of the co-occurrence matrix for the adjectives survey, including group means and bootstrap-derived confidence intervals, colored by nationality. Participants who responded to the Nationality question with something other than French, American, or an American-other combination were grouped together for the third group. Axis labels indicate dimension, eigenvalue, and explained variance for that dimension."
...
muslabs <- paste("Ex", c(1:16,18:30), sep = ".")
...
adjbrick.AM <- adjbrick[,,n.rowvec$US]
adj.cont.AM <- apply(adjbrick.AM, c(1,2), sum)

adjbrick.FR <- adjbrick[,,n.rowvec$FR]
adj.cont.FR <- apply(adjbrick.FR, c(1,2), sum)

adjbrick.o <- adjbrick[,,n.rowvec$Other]
adj.cont.o <- apply(adjbrick.o, c(1,2), sum)


adj4MFA <- abind(adj.cont.FR, adj.cont.AM, adj.cont.o, along = 2)

frvec <- t(as.data.frame(rep("France", length.out = dim(adj.cont.FR)[2])))
amvec <- t(as.data.frame(rep("US", length.out = dim(adj.cont.AM)[2])))
o.vec <- t(as.data.frame(rep("Other", length.out = dim(adj.cont.o)[2])))

colnames(frvec) <- colnames(o.vec) <- colnames(amvec) <- colnames(adj.cont.FR)
rownames(frvec) <- rownames(amvec) <- rownames(o.vec) <- "group"

mfagroups <- abind(frvec, amvec, o.vec, along = 2)
...
pfi4pfs[,2,c(1:3)] <- pfi4pfs[,2,c(1:3)]*-1

pfi4pfs <- renameCols(pfi4pfs)
rownames(pfi4pfs) <- c(1:16,18:30)
dimnames(pfi4pfs)[[3]] <- c("France", "US", "Other")
...
dimnames(pfi4pfs.t)[[3]] <- c("France", "US", "Other")
